...
/datum/game_mode/malfunction/verb/basic_encryption_hack(obj/machinery/power/apc/A as obj in get_unhacked_apcs(src))
	...
	set desc = "10 CPU - Basic encryption hack that allows you to overtake APCs on the colony."
	var/price = 10
...
/datum/game_mode/malfunction/verb/elite_encryption_hack()
	...
	set desc = "200 CPU - Allows you to hack the colony's ALERTCON system, changing alert level. Has high chance of failing."
	var/price = 200
	var/mob/living/silicon/ai/user = usr
	if(!ability_prechecks(user, price))
		return
...
/datum/game_mode/malfunction/verb/system_override()
	...
	set desc = "500 CPU - Begins hacking colony's primary firewall, quickly overtaking remaining APC systems. When completed grants access to colony's self-destruct mechanism. Network administrators will probably notice this."
	var/price = 500
	var/mob/living/silicon/ai/user = usr
	...
	if(duration > 3000)							// Two types of announcements. Short hacks trigger immediate warnings. Long hacks are more "progressive".
		spawn(0)
			...
			if(!user || user.stat == DEAD)
				return
			command_announcement.Announce("We started tracing the intruder. Whoever is doing this, they seem to be on the colony itself. We suggest checking all network control terminals. We will keep you updated on the situation.", "Network Monitoring")
			sleep(duration/5)
			if(!user || user.stat == DEAD)
				return
...
=======
	command_announcement.Announce("Our system administrators just reported that we've been locked out from your control network. Whoever did this now has full access to the colony's systems.", "Network Administration Center")
